...
function LeadCard({ id, name, company, valueBrl, probability, email, phone, source }: { id: string; name: string; company?: string; valueBrl: number; probability: number; email?: string; phone?: string; source: string }) {
...
function Column({ stage, children }: { stage: LeadStage; children: React.ReactNode }) {
  ...
  return (
    <section ref={setNodeRef} className={`min-h-[70vh] w-[260px] shrink-0 rounded-xl border p-2 ${isOver ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-slate-100'}`}>
...
export function PipelinePage() {
  ...
  const onDragEnd = ({ active, over }: DragEndEvent) => {
    if (!over) return;
    moveLead(String(active.id), over.id as LeadStage);
  };
          ...
          {STAGES.map((stage) => {
            ...
            return (
              <Column key={stage} stage={stage}>
                ...
                {stageLeads.map((lead) => (
                  <LeadCard key={lead.id} id={lead.id} name={lead.name} company={lead.company} valueBrl={lead.valueBrl} probability={lead.probability} email={lead.email} phone={lead.phone} source={lead.source} />
                ))}
              </Column>
            );
          })}
